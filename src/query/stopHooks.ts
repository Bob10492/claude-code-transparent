  ...
  if (!isBareMode()) {
    ...
    if (
      ...
      void import('../services/extractMemories/extractMemories.js')
        .then(extractMemoriesModule => {
          if (typeof extractMemoriesModule.executeExtractMemories !== 'function') {
            logForDebugging(
              '[stopHooks] executeExtractMemories export missing; skipping auto-memory extraction',
            )
            return
          }
        ...
        .catch(error => {
          logForDebugging(
            `[stopHooks] failed to load extractMemories: ${String(error)}`,
          )
        })
    }
